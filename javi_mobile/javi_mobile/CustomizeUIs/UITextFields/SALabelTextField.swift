...
@IBDesignable
open class SALabelTextField: UITextField, Validatable {
    ...
    @objc dynamic open var titleFont: UIFont = UIFont.robotoBold(size: 16) ?? .systemFont(ofSize: 13) {
        didSet {
            updateTitleLabel()
        }
    }
    ...
    @IBInspectable dynamic open var titleColor: UIColor = .lightGray {
        didSet {
            updateTitleColor()
        }
    }
    ...
    @IBInspectable dynamic open var selectedTitleColor: UIColor = .yellow {
        didSet {
            updateTitleColor()
        }
    }
    ...
    @IBInspectable dynamic open var selectedLineColor: UIColor = .yellow {
        didSet {
            updateLineView()
        }
    }
    ...
    */
    ...
    fileprivate final func init_SALabelTextField() {
        borderStyle = .none
        createTitleLabel()
        createLineView()
        updateColors()
    ...
    fileprivate func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(SALabelTextField.editingChanged), for: .editingChanged)
    }
    ...
    */
    ...
    override open func becomeFirstResponder() -> Bool {
        ...
        errorMessage = nil //reset error message when user start input on textfield
        updateControl(true)
        return result
    }
    ...
    fileprivate func updateTitleColor() {
        if !isEnabled {
            ...
            if editingOrSelected { //removed || isHighlighted
    ...
    @IBInspectable
    open var titleVisible: Bool = true {
        didSet {
            if titleVisible == false {
                titleLabel.isHidden = true
            }
            updateTitleColor()
            updateTitleVisibility(true, completion: nil)
        }
    }
    ...
    */
    ...
    open func isTitleVisible() -> Bool {
        return hasText || hasErrorMessage || titleVisible
    }
    ...
    */
    ...
    func isValid(allowEmpty: Bool = true) -> Bool {
        self.reset()
        if let text = self.text?.trimmingCharacters(in: .whitespacesAndNewlines), text.characters.count <= Constants.textLengthLimitDefault {
            if allowEmpty {
                return true
            }
            else {
    ...
    override open func awakeFromNib() {
        super.awakeFromNib()
        self.tfMaxLength = Constants.textLengthLimitDefault
    }
    
//    /// should Change Characters In range
//    ///
//    /// to support limited text length for text field
//    ///
//    /// - Parameters:
//    ///   - textField: textField editing
//    ///   - range: range
//    ///   - string: replacement String
//    /// - Returns: is alow change or not
...
//        let newLength = text.characters.count + text.characters.count
//        reset()
//        if newLength > Constants.textLengthLimitDefault {
//            notifyInvalid()
//        }
//        return newLength <= Constants.textLengthLimitDefault
//    }
} // swiftlint:disable:this file_length

private var maxLengths = [UITextField: Int]()
// 2
extension SALabelTextField {
    
    // 3
    @IBInspectable var tfMaxLength: Int {
        get {
            // 4
            guard let length = maxLengths[self] else {
                return Constants.textLengthLimitDefault
            }
            return length
        }
        set {
    ...
    @objc func limitLength(textField: UITextField) {
        // 6
        guard let prospectiveText = textField.text, prospectiveText.characters.count >= tfMaxLength else {
            _ = self.isValid()
            return
        }
        ...
        // 7
        text = String(prospectiveText.prefix(tfMaxLength))
